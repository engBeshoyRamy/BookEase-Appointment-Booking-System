...
function AppContent() {
      ...
      case 2:
        return selectedService ? (
          <DateTimeSelection
            ...
            onSelectDate={(date) =>
              setBookingData({ ...bookingData, date: formatDate(date) })
            }
            onSelectTime={(time) =>
  ...
  const renderPage = () => {
    if (!services) {
      return <LoadingSpinner fullScreen size="lg" text="Loading services..." />;
    }
    switch (currentPage) {
      case "home":
        return (
                  ...
                  {[1, 2, 3, 4].map((step) => (
                    <div key={step} className="flex items-center">
                      ...
                      {step < 4 && (
                        <div
                          className={`w-12 h-1 ${
                            step < bookingStep ? "bg-green-600" : "bg-gray-300"
                          }`}
                        />
                      )}
                    </div>
